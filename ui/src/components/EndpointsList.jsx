...
function EnpointsList() {
  ...
  function openEndpoint(endpoint) {
    const { showAllEndpointsButtonRef, refreshRef, stack, stackPtr } =
      controlsRef.current

    showAllEndpointsButtonRef.current.innerHTML =
  ...
  function renderList() {
    if (showAllEndpointsFlag) {
      ...
      return (
        <ul key="ul" style={{ listStyleType: 'none', paddingLeft: '0px' }}>
          {data.allEndpoints.map((el, i) => (
            <section
              className="endpoint"
              onClick={() => openEndpoint(el)}
              style={{ padding: '7px 0px 0px 0px' }}
            >
              <section>
                <Flex width="100%" justify="between">
                  <Flex justify="center" direction="column">
                    <li key={i} className="endpointText">
                      {el.url}
                    </li>
                  </Flex>
                  <Flex justify="end">
